...
def get_nearest_stops(route_short_name, user_lat, user_lon):
    ...
    for rid, rname in routes:
        if rname == route_short_name or (
            len(rname) > len(route_short_name)
            and rname[len(route_short_name)].isalpha()
        ):
    ...
    for direction_stops in [dir_0, dir_1]:
        if direction_stops:
            nearest = min(
                direction_stops, key=lambda s: haversine(user_lat, user_lon, s[2], s[3])
            )
            best_stops.append(
                {
                    "stop_id": nearest[0],
...
async def fetch_trip_updates(route_ids, stops):
    ...
    Returns tuple: ({stop_id: [arrival_minutes, ...]}, feed_age_seconds)
    """
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.get(
                ...
                # Latency: seconds since the feed was generated by the agency
                feed_ts = feed.header.timestamp
                feed_age_sec = int(now - feed_ts) if feed_ts else None
                if feed_age_sec is not None:
                    logger.info(f"tripUpdates feed age: {feed_age_sec}s")
...
async def fetch_realtime_vehicles(route_ids):
    ...
    Returns tuple: (list of vehicle dicts, feed_age_seconds)
    """
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.get(
                ...
                feed_age_sec = int(now - feed_ts) if feed_ts else None
                if feed_age_sec is not None:
                    logger.info(f"vehiclePositions feed age: {feed_age_sec}s")
...
def get_fallback_speed_kmh() -> float:
    """Return estimated bus speed (km/h) based on Buenos Aires time-of-day.

    Morning/evening peak hours see slower speeds due to traffic congestion.
    Midday and overnight are faster.
    """
    hour = datetime.now(ZoneInfo("America/Argentina/Buenos_Aires")).hour
    if 7 <= hour <= 9:
        return 14.0  # Morning peak
    elif 10 <= hour <= 16:
        return 22.0  # Midday valley
    elif 17 <= hour <= 20:
        return 13.0  # Evening peak
    else:
        return 28.0  # Overnight / early morning


def calculate_eta_linear(dist_km) -> int:
    """Calculate ETA using time-of-day speed estimate (last fallback)."""
    speed = get_fallback_speed_kmh()
    return max(1, round((dist_km / speed) * 60))


def detect_bunching(vehicles, threshold_km: float = 0.3) -> bool:
    """Detect bus bunching: two vehicles on the same route+direction within threshold_km.

    Only meaningful when operating on a filtered list of vehicles for one route/stop.
    """
    for i, v1 in enumerate(vehicles):
        for v2 in vehicles[i + 1 :]:
            if v1["route_id"] != v2["route_id"]:
                continue
            if v1["direction_id"] is not None and v2["direction_id"] is not None:
                if v1["direction_id"] != v2["direction_id"]:
                    continue
            dist = haversine(
...
async def get_etas_for_stops(stops):
    ...
    Each result entry includes:
      ...
      - source: 'realtime' | 'estimated' | 'none'
      - data_age_sec: seconds since feed was generated (None if unavailable)
      - bunching_warning: True if two buses on same route/direction are < 300m apart
    """
    route_ids = list({s["route_id"] for s in stops})

    # Primary: tripUpdates (real-time arrival predictions from agency)
    trip_updates, tu_age = await fetch_trip_updates(route_ids, stops)
    ...
    vehicles, vp_age = await fetch_realtime_vehicles(route_ids)
    if not vehicles:
    ...
    for stop in stops_needing_fallback:
        ...
        direction_matched = [
            v for v in route_vehicles if v["direction_id"] == stop["direction_id"]
        ]
        ...
        for v in route_vehicles:
            v["dist"] = haversine(
                v["latitude"], v["longitude"], stop["lat"], stop["lon"]
            )
        ...
        # Detect bus bunching before computing ETAs
        bunching = detect_bunching(route_vehicles)
        if bunching:
            logger.info(
                f"Bus bunching detected for stop {stop['stop_id']} route {stop['route_id']}"
            )
        ...
        for v in route_vehicles:
            ...
            if eta is None:
                # Try OSRM
                eta = await calculate_eta_osrm(
                    v["latitude"], v["longitude"], stop["lat"], stop["lon"]
                )
            if eta is None:
                # Linear fallback (time-of-day aware)
        ...
        try:
            ...
            if location:
                return await process_location(
                    update, context, location.latitude, location.longitude
                )
            else:
                await update.message.reply_text(
                    "No encontré esa dirección en CABA. Enviame tu ubicación GPS."
                )
        except Exception as e:
            if "malformed" in str(e):
                logger.error(f"Critical DB error: {e}")
                await update.message.reply_text(
                    "Error en la base de datos del servidor. Reintentando..."
                )
            else:
                logger.error(f"Geocoder error: {e}")
                await update.message.reply_text(
                    "Error al buscar dirección. Enviame tu ubicación GPS."
                )
...
async def process_location_update(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    return await process_location(
        update, context, user_location.latitude, user_location.longitude
    )


async def process_location(
    update: Update, context: ContextTypes.DEFAULT_TYPE, lat, lon
):
    line = context.user_data.get("current_line")
    await update.message.reply_text(
        f"Buscando paradas cercanas del {line}...", reply_markup=ReplyKeyboardRemove()
    )

    stops = get_nearest_stops(line, lat, lon)
    if not stops:
        await update.message.reply_text(
            f"No encontré paradas para la línea {line} cerca de tu posición (radio 5km)."
        )
    ...
    for stop in stops:
        headsign = (
            stop["headsign"].title() if stop["headsign"] else "Destino Desconocido"
        )
        ...
        bunching = data.get("bunching_warning", False)
        ...
        if not etas:
            ...
            # Data freshness indicator (show if data is older than 90 seconds)
            if age is not None and age > 90:
                age_min = round(age / 60)
                response_text += f"_\u26a0\ufe0f Datos de hace {age_min} min_\n"

            # Bus bunching warning
            if bunching:
                response_text += (
