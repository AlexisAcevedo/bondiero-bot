...
def download_gtfs():
    print("Descargando GTFS a disco...")
    params = {"client_id": CLIENT_ID, "client_secret": CLIENT_SECRET}
    headers = {"User-Agent": "Mozilla/5.0"}

    with requests.get(
        GTFS_URL, params=params, headers=headers, stream=True, timeout=120
    ) as r:
        r.raise_for_status()
...
def build_database():
    ...
    with zipfile.ZipFile(ZIP_FILE) as zf:
        ...
        cursor.execute(
            "CREATE TABLE trips (route_id TEXT, trip_id TEXT, direction_id INTEGER, trip_headsign TEXT)"
        )
        ...
        for row in reader:
            if row["trip_id"] in valid_trip_ids:
                batch.append(
                    (row["trip_id"], row["stop_id"], int(row["stop_sequence"]))
                )
                if len(batch) >= 10000:
